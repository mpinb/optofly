...
def test_toml_parsed_once_across_many_config_objects(monkeypatch):
    ...
    ~3000 tomllib.load calls for one LiquidLensConfig.from_path(); it must be 1."""
    call_count = {"n": 0}
    ...
    LiquidLensConfig.from_path("configs/config.example.toml")
...
def test_trigger_handler_config_still_gets_fov_from_camera_config():
    ...
    trigger_cfg = TriggerHandlerConfig.from_path("configs/config.example.toml")
    camera_cfg = CameraConfig.from_path("configs/config.example.toml")
