...
def test_texture_dimensions():
    img = _generate_random_texture(
        ...
        fg_color=(0, 0, 0), bg_color=(255, 255, 255), seed=42,
    )
...
def test_texture_reproducible_with_same_seed():
    kwargs = dict(width=192, height=108, square_size_px=20, density=0.5,
                  fg_color=(0, 0, 0), bg_color=(255, 255, 255), seed=42)
...
def test_texture_differs_with_different_seeds():
    img1 = _generate_random_texture(192, 108, 20, 0.5, (0,0,0), (255,255,255), seed=1)
    img2 = _generate_random_texture(192, 108, 20, 0.5, (0,0,0), (255,255,255), seed=2)
    diffs = sum(
        1 for x in range(0, 192, 8) for y in range(0, 108, 8)
...
def test_texture_density_zero_gives_all_background():
    img = _generate_random_texture(64, 64, 8, density=0.0,
                                   fg_color=(0, 0, 0), bg_color=(255, 255, 255), seed=0)
    for x in range(64):
        for y in range(64):
...
def test_texture_density_one_gives_all_foreground():
    img = _generate_random_texture(64, 64, 8, density=1.0,
                                   fg_color=(0, 0, 0), bg_color=(255, 255, 255), seed=0)
    for x in range(64):
        for y in range(64):
