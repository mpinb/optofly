...
pub fn write_csv(buffer: &FrameBuffer, csv_path: &str, trigger_frame_idx: Option<u64>) -> Result<(), String> {
    let file = File::create(csv_path)
        .map_err(|e| format!("Cannot create CSV {}: {}", csv_path, e))?;
    let mut wtr = csv::Writer::from_writer(file);

    wtr.write_record(["frame_idx", "nframe", "ts_sec", "ts_usec", "cam_time_ns", "trigger_frame_idx"])
