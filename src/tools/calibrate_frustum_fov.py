...
Requires: Ximea camera connected, BRAID ZMQ publisher running, liquid lens
connected with an existing liquid_lens.csv calibration.
...
from src.utils.config import LiquidLensConfig, ZMQConfig


# ---------------------------------------------------------------------------
# BRAID tracker thread (ZMQ subscriber)
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    tracker = _BraidTracker(zmq_cfg.braid_port, stop_event)
    tracker.start()
    print(f"  Subscribed to BRAID on port {zmq_cfg.braid_port}")
