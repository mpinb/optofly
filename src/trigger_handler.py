...
HEADING_THRESHOLD = np.pi / 4  # Maximum angle difference to consider "heading toward center"
...
class TrackedObject:
    ...
    velocities: deque = field(default_factory=lambda: deque(maxlen=HEADING_HISTORY_SIZE))
    timestamps: deque = field(default_factory=lambda: deque(maxlen=HEADING_HISTORY_SIZE))
    ...
    def update(self, x: float, y: float, z: float, 
               xvel: float, yvel: float, zvel: float, 
...
class TriggerHandler(mp.Process):
    ...
    def __init__(self, config_path: str = "config.toml", event: Optional[mp.Event] = None):
        """
    ...
    def _initialize_zmq(self) -> None:
        ...
        try:
            ...
            publisher_address = self.config.zmq.get_publisher_address(self.config.zmq.trigger_port)
            ...
            subscriber_address = self.config.zmq.get_subscriber_address(self.config.zmq.braid_port)
            ...
            self.subscriber.setsockopt_string(zmq.SUBSCRIBE, self.config.zmq.braid_topic)
    ...
    def is_in_trigger_zone(self, x: float, y: float, z: float) -> bool:
        ...
        return (distance <= self.config.radius and 
                self.config.z_lim[0] <= z <= self.config.z_lim[1])
    ...
    def _process_birth(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Started tracking object {obj_id} at position "
    ...
    def _process_update(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if obj_id not in self.tracked_objects:
                logger.warning(f"Received Update for unknown object {obj_id}, creating new tracking entry")
    ...
    def _send_trigger(self, obj_id: int) -> None:
        ...
        if not self.config.opto_trigger_active:
            logger.debug(f"Stimulation trigger skipped for object {obj_id} (opto_trigger not active)")
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", "-c", default="config.toml", help="Path to config file")
