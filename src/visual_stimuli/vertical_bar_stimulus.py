...
class VerticalBarStimulus(BaseStimulus):
    ...
    def __init__(
        ...
        self.positions_deg = self._parse_positions(
            config.get("positions_deg", [90])
        )
    ...
    def on_trigger(self, trigger_data: Dict[str, Any]) -> None:
        ...
        self.display_duration_ms = np.random.choice(
            self.display_duration_ms_options
        )
        
        # Randomly select cooldown duration from options
        self.cooldown_duration_ms = np.random.choice(
            self.cooldown_duration_ms_options
        )
    ...
    def _needs_wrapping(self) -> bool:
        ...
        return (
            (self.bar_x < 0) or                    # Left edge off screen
            (bar_right_edge > screen_width)        # Right edge off screen
        )
    ...
    def _log_stimulus_event(
        ...
        log_data = {
            ...
            "braid_timestamp": trigger_data["braid_timestamp"],  # From tracking
            "trigger_timestamp": trigger_data["trigger_timestamp"],  # From trigger handler
