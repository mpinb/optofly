...
This file provides guidance to Claude Code or GPT-5-CODEX when working with code in this repository.
...
### Liquid Lens Kalman filter
Implementing a Kalman filter in the LiquidLens class would provide more accurate lens control by combining measured positions with predicted future positions. Here's a high-level implementation strategy:

  1. State Representation:
    - State vector: [x, y, z, vx, vy, vz]
    - Track position and velocity in 3D space
    - Use existing velocity data from BRAID messages
  2. Filter Implementation:
    - Create a KalmanFilter class that maintains state for each tracked object
    - Initialize filter when a new object is tracked
    - Update filter with each new position measurement
    - Use filter to predict position at time of next lens adjustment
  3. Integration Points:
    - Add a dictionary to store Kalman filters for each tracked object_id
    - Update the filter in run() when receiving position updates
    - Add time-aware prediction that accounts for system latency
    - Use predicted position to set lens focus ahead of current position
  4. Latency Compensation:
    - Calculate system latency (message processing + lens adjustment time)
    - Predict object position at (current_time + latency)
    - Set lens focus based on predicted position rather than measured position
  5. Configuration Options:
    - Add Kalman filter parameters to config.toml
  ...
  This approach would particularly benefit fast-moving objects where the delay between position measurement and lens adjustment is significant. By predicting where the object will be when the lens actually focuses, tracking accuracy would improve substantially.
...
## Dev
To run ruff, always use `uvx ruff ...`